...
静默施法 + 反制法术 -> 反制法术触发条件 + 微妙法术成分机制 + 施法构材可观察性
盲视 + 隐形 -> 隐形状态规则 + 盲视感官规则
```
...
- 本轮收紧回答 prompt 并加入 `spell_component_observability` 后，`Memory` 分数达到 `2.00`。
- `静默施法 + 反制法术` 现在会显式暴露 `施法构材可观察性` missing，避免把普通“施法”动作误当成足够证据。
